...
export default function PersonalizationSettings({ onClose }: PersonalizationSettingsProps) {
  ...
  return (
    <div className="bg-black/40 backdrop-blur-lg border border-momentum-teal/20 rounded-xl p-6 space-y-6">
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="w-full flex items-center justify-center space-x-2 px-6 py-3 momentum-grad-interactive rounded-lg text-white font-semibold hover:scale-105 transition-transform disabled:opacity-50 disabled:hover:scale-100"
      >
        {saving ? (
          <>
            <Loader2 className="w-5 h-5 animate-spin" />
